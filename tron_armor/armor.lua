...
ARMOR_MATERIALS = {
	tron = "trongrid:tron_crystal",
}
ARMOR_FIRE_PROTECT = minetest.get_modpath("trongrid") ~= nil
ARMOR_FIRE_NODES = {
...
if not minetest.get_modpath("trongrid") then
	ARMOR_MATERIALS.tron = nil
end
armor = {
	timer = 0,
...
elseif minetest.get_modpath("unified_inventory") then
	...
	unified_inventory.register_page("armor", {
		get_formspec = function(player, perplayer_formspec)
			...
			local formspec = "background[0.06,"..fy..";7.92,7.52;tron_armor_ui_form.png]"
				.."label[0,0;Armor]"
...
armor.set_player_armor = function(self, player)
	...
	local armor_texture = "tron_armor_trans.png"
	local armor_level = 0
	local armor_heal = 0
	local armor_fire = 0
	local armor_water = 0
...
armor.get_armor_formspec = function(self, name)
	if not armor.textures[name] then
		minetest.log("error", "tron_armor: Player texture["..name.."] is nil [get_armor_formspec]")
		return ""
	end
	if not armor.def[name] then
		minetest.log("error", "tron_armor: Armor def["..name.."] is nil [get_armor_formspec]")
		return ""
	end
...
armor.get_valid_player = function(self, player, msg)
	msg = msg or ""
	if not player then
		minetest.log("error", "tron_armor: Player reference is nil "..msg)
		return
	end
	local name = player:get_player_name()
	if not name then
		minetest.log("error", "tron_armor: Player name is nil "..msg)
		return
	end
	...
	if not pos then
		minetest.log("error", "tron_armor: Player position is nil "..msg)
		return
	elseif not player_inv then
		minetest.log("error", "tron_armor: Player inventory is nil "..msg)
		return
	elseif not armor_inv then
		minetest.log("error", "tron_armor: Detached armor inventory is nil "..msg)
		return
	end
...
default.player_register_model("tron_armor_character.b3d", {
	animation_speed = 30,
	textures = {
		armor.default_skin..".png",
		"tron_armor_trans.png",
		"tron_armor_trans.png",
	},
	animations = {
		stand = {x=0, y=79},
...
minetest.register_on_joinplayer(function(player)
	default.player_set_model(player, "tron_armor_character.b3d")
	local name = player:get_player_name()
	local player_inv = player:get_inventory()
	...
	armor.textures[name] = {
		...
		armor = "tron_armor_trans.png",
		wielditem = "tron_armor_trans.png",
...
if ARMOR_FIRE_PROTECT == true then
	...
	print ("[tron_armor] Fire Nodes disabled")
end
